class FastImage
  struct WEBP < Meta
    ...
    private def decode_vp8(io : IO)
      ...
      @width = IO::ByteFormat::LittleEndian.decode(UInt16, io).to_u16
      @height = IO::ByteFormat::LittleEndian.decode(UInt16, io).to_u16
    end
    ...
    private def decode_vp8l(io : IO)
      ...
      tmp = tmp.map { |element| element.to_u16 }

      @width = (1 + (((tmp[1] & 0x3f) << 8) | tmp[0])).to_u16
      @height = (1 + (((tmp[3] & 0xF) << 10) | (tmp[2] << 2) | ((tmp[1] & 0xC0) >> 6))).to_u16
    end
    ...
    private def decode_vp8x(io : IO)
      ...
      @width = (1 + tmp[0] + (tmp[1] << 8) + (tmp[2] << 16)).to_u16
      @height = (1 + tmp[3] + (tmp[4] << 8) + (tmp[5] << 16)).to_u16
    end
  end
end
